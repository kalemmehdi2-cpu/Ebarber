    ...
    <div style={{ background: '#F8F9FF', minHeight: '100vh', fontFamily: 'Inter, sans-serif', color: '#0A0F2C', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .fu1 { animation: fadeUp 0.7s ease forwards; }
        .fu2 { animation: fadeUp 0.7s ease 0.15s forwards; opacity: 0; }
        .fu3 { animation: fadeUp 0.7s ease 0.3s forwards; opacity: 0; }
        .fu4 { animation: fadeUp 0.7s ease 0.45s forwards; opacity: 0; }
        ...
        .btn-p { transition: all 0.2s ease; }
        .btn-p:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(10,15,44,0.25); }
        ...
        @media (max-width: 768px) {
          ...
          .stats-grid { grid-template-columns: repeat(2, 1fr) !important; gap: 24px !important; padding: 60px 20px !important; }
          .coiffeurs-grid { grid-template-columns: repeat(2, 1fr) !important; gap: 14px !important; }
      ...
      {/* NAV */}
      <nav style={{
        height: 64, position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200,
        ...
        <div className="nav-inner" style={{ maxWidth: 1100, margin: '0 auto', padding: '0 40px', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="nav-links" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <Link href="/auth/register" className="btn-p" style={{ background: '#0A0F2C', color: '#fff', padding: '11px 22px', borderRadius: 12, textDecoration: 'none', fontSize: 14, fontWeight: 700 }}>
              S'inscrire →
            </Link>
          </div>
          <Link href="/auth/register" className="nav-mobile-btn" style={{ background: '#0A0F2C', color: '#fff', padding: '9px 18px', borderRadius: 10, textDecoration: 'none', fontSize: 13, fontWeight: 700 }}>
            S'inscrire
          </Link>
        </div>
      </nav>
      ...
      {/* HERO */}
      <section style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0A0F2C 0%, #1a2560 50%, #0d1b4e 100%)', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', paddingTop: 64 }}>
        ...
        <div className="float" style={{ position: 'absolute', top: '15%', right: '10%', fontSize: 60, opacity: 0.1 }}>✂️</div>
        <div className="float2" style={{ position: 'absolute', bottom: '20%', right: '6%', fontSize: 44, opacity: 0.08 }}>💈</div>

        <div className="hero-grid" style={{ maxWidth: 1100, margin: '0 auto', padding: '80px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center', width: '100%' }}>
          ...
          <div className="hero-text">
            ...
            <div className="fu4 hero-btns" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 36 }}>
              <Link href="/auth/register" className="btn-p" style={{ background: '#fff', color: '#0A0F2C', padding: '15px 28px', borderRadius: 14, textDecoration: 'none', fontSize: 15, fontWeight: 800, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                ✂️ Réserver maintenant
              </Link>
              <Link href="/auth/register?role=coiffeur" style={{ background: 'rgba(255,255,255,0.1)', color: '#fff', padding: '15px 24px', borderRadius: 14, textDecoration: 'none', fontSize: 14, fontWeight: 600, border: '1px solid rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)' }}>
                Devenir coiffeur →
              </Link>
            </div>
          ...
          <div className="hero-cards" style={{ position: 'relative', height: 480, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {/* Main card */}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: '1px solid rgba(255,255,255,0.1)', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: 12 }}>
                {[['4.9','Note'],['312','Avis'],['Dispo','Statut']].map(([v,l]) => (
                  <div key={l} style={{ textAlign: 'center' }}>
                    <div style={{ color: v === 'Dispo' ? '#4ade80' : '#fff', fontWeight: 700, fontSize: 13 }}>{v === 'Dispo' ? '● '+v : v}</div>
                    <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10 }}>{l}</div>
                  </div>
                ))}
              </div>
              <div style={{ background: 'linear-gradient(135deg, #3B5BDB, #7B9EFF)', borderRadius: 10, padding: '11px', textAlign: 'center', color: '#fff', fontWeight: 700, fontSize: 13 }}>
                📅 Réserver
              </div>
            </div>
      ...
      {/* COIFFEURS CARDS */}
      <section className="section-pad" style={{ padding: '100px 40px', background: '#F8F9FF' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <div style={{ display: 'inline-block', background: '#EEF0FF', color: '#3B5BDB', fontSize: 12, fontWeight: 700, padding: '6px 16px', borderRadius: 20, marginBottom: 14, letterSpacing: 0.5 }}>✂️ NOS COIFFEURS</div>
          ...
          <div style={{ textAlign: 'center', marginTop: 36 }}>
            <Link href="/explore" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: '#3B5BDB', textDecoration: 'none', fontSize: 14, fontWeight: 700, border: '1px solid #3B5BDB', padding: '12px 28px', borderRadius: 12 }}>
      ...
      {/* HOW IT WORKS */}
      <section className="section-pad" style={{ padding: '100px 40px', background: '#0A0F2C' }}>
      ...
      {/* CTA */}
      <section className="section-pad" style={{ padding: '100px 40px', background: '#F8F9FF' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', background: 'linear-gradient(135deg, #0A0F2C 0%, #1a2560 100%)', borderRadius: 28, position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="cta-box" style={{ padding: '64px 56px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 16, lineHeight: 1.7, marginBottom: 32, maxWidth: 440, margin: '0 auto 32px' }}>
              Rejoignez +500 coiffeurs sur E-Barber. Gérez vos horaires, prestations et revenus depuis votre dashboard.
            </p>
